...
export function ActionableInsight({ title, insight, action, status }: ActionableInsightProps) {
  return (
    <Card className={`border-l-4 overflow-hidden ${
      status === "success" 
        ? "border-l-green-500 dark:border-green-600" 
        : status === "warning" 
      ...
      <CardContent className="p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-600 dark:text-gray-300 mt-1">{insight}</p>
            
            <div className="mt-4 flex items-center">
              <ArrowRight className="h-4 w-4 mr-2 text-blue-600 dark:text-blue-400" />
              <p className="text-blue-800 dark:text-blue-400 font-medium">{action}</p>
            </div>
          </div>
        </div>
      </CardContent>
